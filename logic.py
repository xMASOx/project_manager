...
class DB_Manager:
    ...
    def add_photo_column(self):
        ...
        if 'photo' not in columns:
            ...
            print("Столбец 'photo' успешно добавлен!")
        else:
...
if __name__ == '__main__':
    ...
    manager.add_photo_column()  # <--- Вставили здесь
    manager.default_insert()

# Тесты
    user_id = 1
    status_id = manager.get_status_id("В процессе разработки")
    project_data = [(user_id, 'Test Project', 'Описание проекта', 'http://example.com', status_id)]
    manager.insert_project(project_data)

    manager.insert_skill(user_id, 'Test Project', 'Python')

    print("Статусы:")
    print(manager.get_statuses())

    print("Проекты пользователя:")
    print(manager.get_projects(user_id))

    print("Информация о проекте:")
    print(manager.get_project_info(user_id, 'Test Project'))

    print("Навыки проекта:")
    print(manager.get_project_skills('Test Project'))

    project_id = manager.get_project_id('Test Project', user_id)
    manager.update_projects('description', ('Новое описание проекта', project_id))

    skill_id = manager._DB_Manager__select_data('SELECT skill_id FROM skills WHERE skill_name = ?', ('Python',))[0][0]
